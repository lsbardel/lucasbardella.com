...
That ratio has flipped. With AI coding agents, I now spend roughly equal time on research and implementation, which is how it should be. The code still needs to be fast, safe, and thoroughly tested. That has not changed. What changed is who does the heavy lifting of turning a well-specified design into a working implementation. I describe the architecture, the interfaces, the performance constraints, and I direct multiple agents in parallel to build out different modules simultaneously. One agent might be implementing an order routing component while another writes the market data normalization layer and a third builds the position reconciliation logic. I review their output, correct course, and integrate the pieces. What used to take me weeks of serial work now happens in days.

The area where AI helps the most is test harness development. Trading systems have a brutal surface area for edge cases. Market data gaps, partial fills, exchange outages, clock drift across venues, currency settlement mismatches, weekend rolls. Writing exhaustive tests for all of this is necessary but tedious work that I never had enough time for. Now I specify the invariants and boundary conditions I care about, and an agent produces hundreds of test scenarios that would have taken me a week to write by hand. The coverage is better than anything I produced before under time pressure.

Here is what this really means in practice: a small team, even a team of two or three people, can now ship production systems that would have previously required a team of ten or fifteen. The hedge fund industry has long been defined by its resource asymmetry. Large firms throw dozens of developers at infrastructure problems while small firms compromise on code quality or move slowly. AI tooling eliminates much of that gap. A small quantitative trading operation, armed with the right agents and a clear architectural vision, can produce and maintain systems that compete with what well-funded multi-strategy platforms deploy. The leverage is enormous. It is not about replacing skilled developers. It is about amplifying what a small, focused team can deliver.
