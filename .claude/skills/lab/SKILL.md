---
name: lab
description: Create a lab entry and the corresponding TypeScript component following the guidelines in this document.
---
...
- **Type-only imports**: TypeScript interfaces and types are erased at compile time and do not exist in the emitted JS. When importing a type from another module, always use `import type` or the inline `import { type Foo }` syntax — otherwise Observable will throw a runtime error trying to resolve a non-existent export:
  ```tsx
  // correct — type is stripped at compile time
  import { type BaseProps, MyComponent } from "./base.js";

  // wrong — Observable will fail at runtime looking for BaseProps
  import { BaseProps, MyComponent } from "./base.js";
  ```
